...
onready var _jwt = get_node("MainContainer/ControlRoom/ServerConnect/Token")
...
func _ready():
	_jwt.set_text(Global.user_token)
	_log.set_text("")
...
func _on_JoinTopic_pressed():
	if not socket:
		...
		if not channel:
			...
			channel = socket.channel(_topic.get_text(), {"game_name": "triad_1"}, presence)
...
func _on_Connect_pressed():
	if not socket:
		socket = PhoenixSocket.new(_host.get_text(), {
			params = {user_id = _username.get_text(), token = _jwt.get_text()}
		})
